...
fn temp_store_dir(label: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("mnemara-file-{label}-{}", uuid::Uuid::new_v4()));
...
async fn changefeed_lists_mutations_and_time_travel_recall_uses_versions() {
    let store = FileMemoryStore::open(FileStoreConfig::new(temp_store_dir("changefeed"))).unwrap();
