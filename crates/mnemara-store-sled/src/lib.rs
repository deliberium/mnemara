...
use mnemara_core::{
    ...
    PortableRecord, PortableStorePackage, RecallExplanation, RecallPlanningTrace, RecallQuery,
    RecallResult, RecallScorer, RecallTraceCandidate, RepairReport, RepairRequest, Result,
    SnapshotManifest, StoreStatsReport, StoreStatsRequest, UpsertReceipt, UpsertRequest,
};
...
impl SledMemoryStore {
    ...
    fn scorer(&self) -> ConfiguredRecallScorer {
        ConfiguredRecallScorer::from_engine_config(&self.engine_config)
    }
                ...
                planning_trace: Some(RecallPlanningTrace {
                    ...
                    candidates: scored
                        ...
                        .map(|(index, (hit, matched_terms))| {
                            let record_id = hit.record.id.clone();
                            ...
                            let mut candidate_channels = if empty_query {
                                vec!["temporal".to_string(), "policy".to_string()]
                            } else {
                                vec!["lexical".to_string(), "policy".to_string()]
                            };
                            if hit.breakdown.semantic > 0.0 {
                                candidate_channels.push("semantic".to_string());
                            }
                            if hit.breakdown.metadata > 0.0 {
                                candidate_channels.push("metadata".to_string());
                            }
                            if hit.breakdown.curation > 0.0 {
                                candidate_channels.push("curation".to_string());
                            }
                            candidate_channels.sort();
                            candidate_channels.dedup();
